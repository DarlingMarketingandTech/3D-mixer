...
// ===================================================================
// OTIMIZAÇÃO: Geometrias, Materiais e Shapes Estáticos Globais
// Movemos para fora dos componentes para evitar Memory Leaks de VRAM
// ===================================================================

// 1. Tonearm Connector
const RIDGE_GEO = new THREE.BoxGeometry(0.05, 0.005, 0.005)
const RIDGE_MAT = new THREE.MeshStandardMaterial({ color: "#333" })

// 2. High Density Knurling & Shared Dummy
const SHARED_DUMMY = new THREE.Object3D()
const KNURLING_GEO = new THREE.BoxGeometry(0.012, 0.012, 0.006)
const KNURLING_MAT = new THREE.MeshStandardMaterial({ color: "#888", metalness: 1, roughness: 0.2 })

// 3. Strobe Dots
const STROBE_DOTS_GEO = new THREE.CylinderGeometry(1, 1, 1, 24)
const STROBE_DOTS_MAT = new THREE.MeshStandardMaterial({ color: "#E0E0E0", roughness: 0.15, metalness: 0.9, envMapIntensity: 0.3 })

// 4. Cueing Lever
const CUEING_LEVER_PATH = new THREE.LineCurve3(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0.08, 0.19))
const CUEING_PADDLE_GEO = (() => {
  const height = 0.155
...
const YOKE_SHAPE = (() => {
  ...
  const w = 0.13, h = 0.24
  shape.moveTo(-w, -h).lineTo(-w, h).lineTo(w, h).lineTo(w, -h).closePath()
  const hole = new THREE.Path()
  const innerW = 0.09, innerH = 0.19
  hole.moveTo(-innerW, -innerH).lineTo(innerW, -innerH).lineTo(innerW, innerH).lineTo(-innerW, innerH).closePath()
...
const TOWER_SHAPE = (() => {
  const shape = new THREE.Shape()
  const hOx = 0.2, vOx = -0.155, tOy = 0.433, bOy = 0.04, tk = 0.05
  const vIx = vOx + tk, tIy = tOy - tk, bIy = bOy + tk
  shape.moveTo(vOx, tOy).lineTo(hOx, tOy).lineTo(hOx, tIy).lineTo(vIx, tIy)
    .lineTo(vIx, bIy).lineTo(hOx, bIy).lineTo(hOx, bOy).lineTo(vOx, bOy).closePath()
  return shape
})()
...
const TONEARM_CURVE = new THREE.CatmullRomCurve3([
  new THREE.Vector3(0, 0, -0.18), new THREE.Vector3(0, 0, 0.3),
  new THREE.Vector3(0.015, 0, 0.68), new THREE.Vector3(0.24, 0, 1.59),
  new THREE.Vector3(0.018, 0, 1.938), new THREE.Vector3(0.02, 0, 1.938),
], false, "centripetal", 0.4)

const TOWER_EXTRUDE_SETTINGS = { steps: 1, depth: 0.08, bevelEnabled: true, bevelThickness: 0.002, bevelSize: 0.002, bevelSegments: 2 }
...
function StrobeLight({ textures, pbr, powerOn = true }: { textures?: LoadedTextures; pbr?: AllPBRTextures; powerOn?: boolean } = {}) {
  ...
  // Item 4: Associa o target apenas 1 vez ao montar, removendo a checagem custosa do useFrame
  useEffect(() => {
...
function StrobeDotsInstanced({ currentSpeed = 0 }: { currentSpeed: number }) {
  ...
  React.useEffect(() => {
    ...
    rows.forEach((row, ri) => {
      ...
      for (let i = 0; i < row.count; i++) {
        ...
        SHARED_DUMMY.position.set(Math.cos(angle) * row.radius, row.y, Math.sin(angle) * row.radius)
        euler.set(STROBE_PLATTER_WALL_ANGLE, -angle + Math.PI / 2, 0, "YXZ")
        SHARED_DUMMY.rotation.copy(euler)
        SHARED_DUMMY.scale.set(scaleX, scaleY, scaleX)
  ...
  useFrame((_, delta) => {
    ...
    rows.forEach((row, ri) => {
      const mesh = rowRefs.current[ri]
      if (mesh) mesh.rotation.y += delta * currentSpeed * row.speedMultiplier
    })
  })

  // Item 2: Usa Geometria e Material declarados globalmente no topo do arquivo
  return (
    <group>
      {rows.map((row, i) => (
        <instancedMesh
          ...
          args={[STROBE_DOTS_GEO, STROBE_DOTS_MAT, row.count]}
        />
      ))}
    </group>
  )
...
function RecordLabel() {
  ...
  const texture = useMemo(() => {
    ...
    ctx.fillText("BY TULIO PORTELA", center - 100, size - 150)
    ...
    const content2 = " 3D PROJECT V0 // 2026 VIBE CODING - DESIGNED BY TULIO PORTELA // ".repeat(2)
...
function SpeedMarkingsVERTICAL() {
  const marks = [
...
function StartStopButtonVERTICAL({ pbr, onToggleStart }: { pbr?: AllPBRTextures; onToggleStart?: () => void } = {}) {
  return (
...
function SpeedButtonsVERTICAL({ pbr }: { pbr?: AllPBRTextures } = {}) {
  return (
...
function PitchFader({ textures }: { textures?: LoadedTextures } = {}) {
  return (
    <group position={[2, 0.1, 0.8]}>
      ...
      <group position={[-0.08, 0.002, 0]}>
        {[
          { z: -0.42, label: "-8" }, { z: -0.315, label: "-6" }, { z: -0.21, label: "-4" }, { z: -0.105, label: "-2" },
          { z: 0, label: "0" }, { z: 0.105, label: "+2" }, { z: 0.21, label: "+4" }, { z: 0.315, label: "+6" }, { z: 0.42, label: "+8" },
        ].map((mark, i) => (
          <group key={i} position={[0, 0, mark.z]}>
            ...
            <Text position={[-0.045, 0, 0]} rotation={[-Math.PI / 2, 0, 0]} fontSize={0.028} color="#d6d6d6" anchorX="right">{mark.label}</Text>
          </group>
        ))}
      </group>
...
function ChassisLogos() {
  return (
    <group position={[2.05, 0.103, 1.68]}>
      <Text position={[-0.11, 0, -0.04]} rotation={[-Math.PI / 2, 0, 0]} fontSize={0.03} color="#7e7e7e" anchorX="center">Special Edition</Text>
      <Text position={[0, 0, 0.08]} rotation={[-Math.PI / 2, 0, 0]} fontSize={0.03} color="#6d6d6d" anchorX="center">Turntable System</Text>
    </group>
  )
}
...
function MetallicLogo() {
  ...
  return (
    <group position={[2.08, 0.101, 1.71]}>
      <mesh rotation={[-Math.PI / 2, 0, 0]}>
      ...
      <group position={[0, 0.04, 0]}>
        ...
        <group rotation={[-0.25, 0, 0]}>
          <group position={[0, 0.009, 0]}>
            {/* Item 2: Usa CUEING_LEVER_PATH global */}
            <Tube args={[CUEING_LEVER_PATH, 4, 0.005, 16, false]}  ><meshStandardMaterial color="#333" metalness={0.7} roughness={0.3} /></Tube>
            <group position={[0, 0.08, 0.19]} rotation={[Math.PI / 3.0, Math.PI, 0]}>
              {/* Item 2: Usa CUEING_PADDLE_GEO global */}
              <mesh geometry={CUEING_PADDLE_GEO}  ><meshStandardMaterial color="#1a1a1a" metalness={0.5} roughness={0.4} /></mesh>
            </group>
          </group>
        </group>
      </group>
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0.204, -0.015]} receiveShadow>
        {/* Item 2: Usa HOOK_SHAPE global */}
        <extrudeGeometry args={[HOOK_SHAPE, { steps: 1, depth: 0.03, bevelEnabled: false }]} />
...
function TonearmConnector() {
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      SHARED_DUMMY.position.set(0, Math.sin(angle) * 0.053, Math.cos(angle) * 0.053)
      SHARED_DUMMY.rotation.set(angle, 0, 0)
      SHARED_DUMMY.updateMatrix()
      ridgesRef.current.setMatrixAt(i, SHARED_DUMMY.matrix)
    }
  ...
  return (
    <group>
      ...
      <group position={[-0.04, 0, 0]}>
        ...
        {/* Item 2: Usa RIDGE_GEO e RIDGE_MAT extraídos para o topo */}
        <instancedMesh ref={ridgesRef} args={[RIDGE_GEO, RIDGE_MAT, 24]} />
      </group>
    </group>
  )
}
  ...
  return (
    <group>
      <mesh castShadow receiveShadow position={[-0.03, 0, -0.04]}>
        {/* Item 2: Usa YOKE_SHAPE global */}
        <extrudeGeometry args={[YOKE_SHAPE, sharpExtrudeSettings]} />
  ...
  return (
    <group position={position}>
      <group>
        ...
        <group position={[0.15, 0.007, 0.12]} rotation={[0, 0.5, 0]}><CueingLever textures={textures} /></group>
        <group position={[0, 0.01, 0.39]} rotation={[0, 0, 0]}><TonearmRest textures={textures} /></group>
      </group>

      <group position={[-0.15, 0, -0.05]} rotation={[0, -0.4, 0]}>
        <mesh castShadow receiveShadow position={[0, 0, -0.04]}>
          {/* Item 2: Usa TOWER_SHAPE e TOWER_EXTRUDE_SETTINGS globais */}
          <extrudeGeometry args={[TOWER_SHAPE, TOWER_EXTRUDE_SETTINGS]} />
          {pbr ? <MetalPaintedPretoPBRMaterial textures={pbr.metalPaintedPreto} /> : <meshStandardMaterial color="#222222" roughness={0.65} metalness={0.8} />}
        </mesh>
...
function ChassisWithHole({ chassisColor, explosionY = 0, pbr }: { chassisColor: string; explosionY?: number; pbr?: AllPBRTextures }) {
  const shape = useMemo(() => {
    ...
    const pfX = 2.05, pfY = 0.8, pfW = 0.16, pfH = 1.035
...
function WirelessController({ currentSpeed = 0 }: { currentSpeed?: number }) {
  ...
  return (
    <group ref={ref} rotation={[-Math.PI / 2, 0, Math.PI / 2]}>
      {/* Item 2: Usa PHASE_GEO global */}
      <mesh geometry={PHASE_GEO} receiveShadow>
    ...
    <group position={[1.64, 0.02, -1]}>
      ...
      <group position={[0, pivotHeight, 0]} ref={armSwingRef} rotation={[0, 0, 0]}>
        ...
        <group rotation={[0.035, 0, 0]}>
          {/* Item 2: Usa TONEARM_CURVE global */}
          <Tube args={[TONEARM_CURVE, 64, 0.046, 32, false]} castShadow>
            {pbr ? (
